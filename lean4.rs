...
impl Expr {
    ...
    /// NF 完全範式 (遞迴求值，用於顯示最終結果)
    fn nf(&mut self, ctx: &Context) {
        self.whnf(ctx); // 先化簡最外層
        match self {
            App(f, arg) => {
                f.nf(ctx);
                arg.nf(ctx); // 遞迴化簡參數
...
fn main() {
    ...
    let bool_lean = r#"
        ...
        (eval (app not (ctor Bool 0))) ; 測試: not true
    "#;
    ...
    let nat_lean = r#"
        ...
        ; let 測試
        (eval (let x Nat (ctor Nat 0) (app (ctor Nat 1) x))) ; 測試: let x = 0 in succ x

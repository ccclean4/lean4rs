...
impl Elaborator {
        ...
        match pre {
            ...
            PreExpr::Var(name) => {
                ...
                if let Some((ind, tag, params, _)) = self.env.ctors.get(name) {
                    ...
                    return Ok(Constructor(ind.clone(), *tag, lvls));
                }
                // 3. Inductive?
                if let Some((params, _)) = self.env.inductives.get(name) {
                    let lvls: Vec<Level> = params.iter()
                        .map(|_| self.fresh_uvar()).collect();
                    return Ok(Inductive(name.clone(), lvls));
                }
                // 4. Definition?
                if let Some((params, _, _)) = self.env.defs.get(name) {
                    let lvls: Vec<Level> = params.iter()
                        .map(|_| self.fresh_uvar()).collect();
                    return Ok(Const(name.clone(), lvls));
                }
            ...
            PreExpr::Lam(bi, param_name, param_ty_pre, body_pre) => {
                ...
                // Expected body type: if we have Pi expected, unwrap it
                ...
                Ok(Lam(bi.clone(), Box::new(pty), Box::new(body)))
            }
            ...
            PreExpr::Pi(bi, param_name, param_ty_pre, body_pre) => {
                ...
                let mut pty = param_ty.clone();
                self.fvar_to_bvar(&mut pty, fvar_id, 0);
                Ok(Pi(bi.clone(), Box::new(pty), Box::new(body)))
            }
            ...
            PreExpr::Let(name, ty_pre, val_pre, body_pre) => {
                ...
                Ok(Let(Box::new(ety), Box::new(eval), Box::new(body)))
            }
            ...
            PreExpr::Fix(fname, ty_pre, body_pre) => {
                ...
                let mut ety = ty.clone();
                self.fvar_to_bvar(&mut ety, fvar_id, 0);
                Ok(Fix(Box::new(ety), Box::new(body)))
            }
            ...
            PreExpr::Match(ind_name, target_pre, motive_pre, branches_pre) => {
                ...
                let motive: Expr = if let Some(m_pre) = motive_pre {
                    ...
                    let ret_ty = self.fresh_mvar(Sort(self.fresh_uvar()));
                ...
                for (ctor_name, binder_names, branch_body_pre) in branches_pre {
                    ...
                    for bname in binder_names {
                        let k = Kernel::new(&self.env);
                        k.whnf(&mut cur_ty);
                        if let Pi(_, param_ty, ret) = cur_ty.clone() {
                    ...
                    // Pop binders in reverse, converting FVars to BVars
                    ...
                    for fid in fvar_ids.iter().rev() {
                        ...
                        branch_body = Lam(BinderInfo::Default, Box::new(bty), Box::new(branch_body));
                    }
    ...
    /// Convert all FVars to BVars using current name_env depth.
    /// This is needed before passing to the kernel.
    fn fvars_to_bvars(&self, expr: &mut Expr) {
        // depth of each FVar id = distance from top of name_env
...
fn fvars_to_bvars_inner(expr: &mut Expr, dm: &HashMap<usize, usize>) {
    match expr {
        FVar(id) => {
            if let Some(depth) = dm.get(id) { *expr = BVar(*depth); }
        }
        BVar(_) | MVar(_) | Sort(_) | Inductive(_, _)
        | Constructor(_, _, _) | Const(_, _) => {}
        App(f, a) => { fvars_to_bvars_inner(f, dm); fvars_to_bvars_inner(a, dm); }
        Lam(_, ty, body) | Pi(_, ty, body) | Fix(ty, body) => {
            fvars_to_bvars_inner(ty, dm);
            // body is under a binder: increment all depths
...
pub fn run_script(script: &str, elab: &mut Elaborator) {
    ...
    while !p.eof() {
        ...
        match cmd.as_str() {
            ...
            "inductive" => {
                let name = p.next_tok();
                // optional universe params:  inductive Foo.{u v} : ...
                let uparams = if p.at("{") {
                    p.consume("{");
                    let mut ps = Vec::new();
